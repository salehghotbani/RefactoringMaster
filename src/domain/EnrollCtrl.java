...
public class EnrollCtrl {
    public void enroll(Student s, List<CSE> courses) {
        try {
            ...
            for (CSE o : courses) {
                ...
                studentPassed(transcript, o);
                //check prerequisite of courses that student requested
                coursesPrerequisite(transcript, o);
                //check time of exam, to not same as another lessons exam time or check if request lesson was taken before
                sameTimeTakenBefore(courses, o);
            }

            GPAChecker(s, courses);
    ...
    private void sameTimeTakenBefore(List<CSE> courses, CSE o) {
        for (CSE o2 : courses) {
            if (o == o2)
                continue;
    ...
    private void coursesPrerequisite(Map<Term, Map<Course, Double>> transcript, CSE o) {
    ...
    private void studentPassed(Map<Term, Map<Course, Double>> transcript, CSE o) {
        for (Map.Entry<Term, Map<Course, Double>> tr : transcript.entrySet()) {
            for (Map.Entry<Course, Double> r : tr.getValue().entrySet()) {
    ...
    private void GPAChecker(Student s, List<CSE> courses) {
